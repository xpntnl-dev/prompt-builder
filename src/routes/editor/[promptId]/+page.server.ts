...
export const load: PageServerLoad = async ({ params, url }) => {
	// Get the version ID from query param (optional)
	const versionIdParam = url.searchParams.get('version');
	...
	// Load ALL versions for this prompt
	...
	if (versionsError || !versions || versions.length === 0) {
		throw error(404, 'No versions found for this prompt');
	}

	// Determine which version to display
	let targetVersion: PromptVersion;
	if (versionIdParam) {
		// Use specified version from query param
		const foundVersion = versions.find((v) => v.id === versionIdParam);
		if (!foundVersion) {
			throw error(404, 'Specified version not found');
		}
		targetVersion = foundVersion;
	} else {
		// Default to active version, or first version if none active
		targetVersion = versions.find((v) => v.is_active) || versions[0];
	}

	// Load ALL sections for the target version (including all variants)
	...
	try {
		...
		console.log(`[Editor] Loaded ${models.length} models from OpenRouter`);
	} catch (err) {
		console.error('[Editor] Failed to load models from OpenRouter:', err);
	}
	...
	return {
		...
		version: targetVersion as PromptVersion,
